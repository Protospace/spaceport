...
export function TransactionEditor(props) {
	...
	const accountOptions = [
		...
		{
			key: '6',
			text: 'Membership Adjustment / Clearing',
			value: 'Clearing',
		},
	...
	const categoryOptions = [
		...
		{
			key: '1',
			text: 'Payment On Account (ie. Course Fee)',
			value: 'OnAcct',
		},
		...
		{
			key: '4',
			text: 'Consumables (Explain in memo)',
			value: 'Consumables',
		},
		{
			key: '5',
			text: 'Purchase of Locker / Goods / Merch / Stock',
			value: 'Purchases',
		},
		//{ key: '6', text: 'Auction, Garage Sale, Nearly Free Shelf', value: 'Garage Sale' },
		{
			key: '7',
			text: 'Reimbursement (Enter a negative value)',
			value: 'Reimburse',
		},
	...
	return (
		<div className="transaction-editor">
			...
			<Form.Group widths="equal">
				<Form.Input label="Date" fluid {...makeProps('date')} />
				<Form.Input label="Amount" fluid {...makeProps('amount')} />
			</Form.Group>
			...
			{input?.account_type !== prevInput?.account_type &&
				input?.account_type === 'PayPal' && (
					<Message visible warning>
						<Message.Header>
							Are you absolutely sure?
						</Message.Header>
						<p>
							PayPal transactions should be automatic. Double
							check there's no duplicate. They may take 24h to
							appear.
						</p>
					</Message>
				)}
			...
			<Form.Input label="Memo / Notes" fluid {...makeProps('memo')} />
		</div>
	);
}
...
function ReportTransaction(props) {
	...
	return (
		<div>
			...
			<p>
				If this transaction was made in error or there is anything
				incorrect about it, please report it using this form.
			</p>
			<p>A staff member will review the report as soon as possible.</p>
			<p>
				Follow up with{' '}
				<a
					href="mailto:directors@protospace.ca"
					target="_blank"
			...
			<Form onSubmit={handleSubmit}>
				<Form.TextArea label="Reason" {...makeProps('report_memo')} />
