	...
	return (
		<Form onSubmit={handleSubmit}>
			...
			<Form.TextArea
				label={
					'Public Bio' +
					(input.public_bio && input.public_bio.length > 400
						? ' — ' + input.public_bio.length + ' / 512'
						: '')
				}
			...
			<p>
				Bio shared with members. Example: contact info, allergies,
				hobbies, etc.
			</p>
			...
			<Form.TextArea
				label={
					'Private Notes' +
					(input.private_notes && input.private_notes.length > 400
						? ' — ' + input.private_notes.length + ' / 512'
						: '')
				}
