...
function MemberInfo(props) {
	...
	const lastCard =
		user.cards &&
		user.cards.sort((a, b) => a.last_seen_at < b.last_seen_at)[0];
		...
		<div>
			...
			{!member.photo_medium && (
				<Message>
					...
					<p>
						<a
							href={staticUrl + '/' + member.member_forms}
							target="_blank"
						>
							Click here
						</a>{' '}
			...
			{!!lastTrans.length && !member.photo_medium && (
				<Message warning>
					...
					<p>
						Visit the <Link to="/account">account settings</Link>{' '}
						page to set one.
					</p>
				</Message>
			)}
			...
			{!lastTrans.length && (
				<React.Fragment>
					...
					<p>
						Create a ${user.member.monthly_fees} / month
						subscription, get your first three months for the price
						of two:
					</p>
					<PayPalSubscribeDeal
...
export function Home(props) {
	...
	const [stats, setStats] = useState(
		JSON.parse(localStorage.getItem('stats', 'false'))
	);
	...
	const getDateStat = (x) =>
		stats && stats[x]
			? moment.utc(stats[x]).tz('America/Edmonton').format('ll')
			: '?';

	const mcPlayers =
		stats && stats['minecraft_players'] ? stats['minecraft_players'] : [];
	const mumbleUsers =
		stats && stats['mumble_users'] ? stats['mumble_users'] : [];

	const getTrackStat = (x) =>
		stats && stats.track && stats.track[x]
			? moment().unix() - stats.track[x]['time'] > 60
				? 'Free'
				: 'In Use'
			: '?';
	const getTrackLast = (x) =>
		stats && stats.track && stats.track[x]
			? moment
					.unix(stats.track[x]['time'])
					.tz('America/Edmonton')
					.format('llll')
			: 'Unknown';
	const getTrackAgo = (x) =>
		stats && stats.track && stats.track[x]
			? moment
					...
					.fromNow()
			: '';
	const getTrackName = (x) =>
		stats && stats.track && stats.track[x] && stats.track[x]['username']
			? stats.track[x]['username']
			: 'Unknown';

	const alarmStat = () =>
		stats && stats.alarm && moment().unix() - stats.alarm['time'] < 300
			? stats.alarm['data'] > 200
				? 'Armed'
				: 'Disarmed'
			: 'Unknown';
			...
			<Grid stackable padded columns={2}>
						...
						<div>
							{bypass_code ? (
								<Message warning>
									...
									<p>
										This page allows you to sign up from
										outside of Protospace.
									</p>
								</Message>
							) : (
								<>
				...
				<Grid.Column>
					<Segment>
						...
						<p>
							Welcome to the Protospace member portal! Here you
							can view member info, join classes, and manage your
							membership.
						</p>
						...
						<p>
							<a
								href="http://wiki.protospace.ca/Welcome_to_Protospace"
								target="_blank"
								rel="noopener noreferrer"
							>
								Protospace Wiki
						...
						<p>
							<a
								href="https://groups.google.com/forum/#!forum/protospace-discuss"
								target="_blank"
								rel="noopener noreferrer"
							>
						...
						<p>
							<a
								href="https://groups.google.com/forum/#!forum/protospace-administration"
								target="_blank"
								rel="noopener noreferrer"
							>
								Admin Google Group
						...
						{!!user && (
							<p>
								<a
									href="https://drive.google.com/open?id=0By-vvp6fxFekfmU1cmdxaVRlaldiYXVyTE9rRnNVNjhkc3FjdkFIbjBwQkZ3MVVQX2Ezc3M"
									target="_blank"
									rel="noopener noreferrer"
								>
									Google Drive
						...
						{!!user && isAdmin(user) && (
							<p>
								<a
									href="https://estancia.hippocmms.ca/"
									target="_blank"
									rel="noopener noreferrer"
								>
						...
						<div>
							...
							<p>
								...
								/>{' '}
								<a
									href="http://games.protospace.ca:8123/?worldname=world&mapname=flat&zoom=3&x=74&y=64&z=354"
									target="_blank"
								>
									[map]
								</a>
