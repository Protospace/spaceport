			...
			<div style={{ padding: '3rem' }}>
				...
				<p>
					{moment
						...
						.format('llll')}
					{num >= 2
						? ', ' + num + ' students sorted by registration time.'
						: '.'}
				</p>
				...
				<Grid stackable padded columns={2}>
					<Grid.Column>
						<Table collapsing unstackable basic="very">
							{clazz.students
								.filter(
									(x) => x.attendance_status !== 'Withdrawn'
								)
								.slice(0, 15)
								.map((x) => (
									<Table.Row key={x.id}>
...
function AttendanceRow(props) {
	...
	return (
		<div className="attendance-row">
			<p>
				<Link to={'/members/' + student.student_id}>
					{student.student_name}
				</Link>
				{student.attendance_status === 'Waiting for payment' &&
					' (Waiting for payment)'}
				:
			</p>

			<Button {...makeProps('Withdrawn')}>Withdrawn</Button>

			<Button {...makeProps('Confirmed')}>Confirmed</Button>

			<Button {...makeProps('Rescheduled')}>Rescheduled</Button>

			<Button {...makeProps('No-show')}>No-show</Button>

			<Button {...makeProps('Attended')}>Attended</Button>
...
function InstructorClassEditor(props) {
	...
	return (
		<div className="class-editor">
			<Form.Input label="Cost ($) — 0 for free" {...makeProps('cost')} />
	...
	return (
		<div>
			...
			{open ? (
				...
				<Button onClick={() => setOpen(true)}>Edit Class</Button>
			)}
		</div>
	);
}
	...
	return (
		<div>
			...
			{!open && success && (
				<p>
					Added!{' '}
					<Link to={'/classes/' + success}>View the class.</Link>
				</p>
			)}
			...
			{open ? (
				...
				<Button onClick={() => setOpen(true)}>Add a Class</Button>
			)}
		</div>
	);
}
